...
douban_comments.fillna("无评价")
print(douban_comments.shape[0])
comments = douban_comments.iloc[:,[2,9,12]]
...
for i in range(0,comments.shape[0]):
	unlabeled_comments.append(comments[i][0])
i = 0
XX = np.empty(len(unlabeled_comments),dtype=list)
for sentence in  unlabeled_comments:
...
for i in range(len(unlabeled_comments)):
	if max(labels[i].tolist()) > 0.5:
		p = labels[i].tolist().index(max(labels[i].tolist()))
	else:
		p = 5
	print('{}   {}		{}'.format(label2word[p], unlabeled_comments[i],labels[i][p]))